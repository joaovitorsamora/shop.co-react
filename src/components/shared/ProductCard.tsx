    ...
    <Link to={`/${productDetailPage}/${product.id}`}>
      <li className="flex gap-3 snap-center min-w-[295px]">
        <article className="m-auto" key={product.id}>
          ...
          <h4 className="text-base font-bold font-poppins pt-2" data-testid="mock-product">
            {product.title}
          </h4>
          <div
          ...
          >
            ...
            <p className="text-xs text-gray-500 font-poppins">{product.review}/5</p>
          </div>
          <div className="flex items-center gap-2">
            <p className="text-xl font-bold text-black font-poppins">${product.price}</p>
            {product.oldPrice && (
              <span className="line-through text-sm text-gray-400 font-poppins">
                ${product.oldPrice}
              </span>
            )}
            {product.discount && (
              <span className="text-sm text-red-500 font-poppins">{product.discount}</span>
            )}
          </div>
        </article>
      </li>
