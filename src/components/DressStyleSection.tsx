...
export const DressStyleSection: React.FC<DressStyleSectionProps> = ({ clothesStyles, title }) => {
  ...
  return (
    <section className="bg-[#f0f0f0] rounded-[20px] mx-4 my-[54px] xl1024:mx-[100px] xl1024:mb-[80px]">
      <h2 className="text-center uppercase text-black text-[2rem] font-bold px-[56px] pt-[40px] pb-[20px]">
        {title}
      </h2>
        ...
        {clothesStyles.map((style, index) => (
          ...
          >
            <a href={style.href}>
              ...
              <img
                src={
                  style.image.includes('-desktop.png')
                    ? style.image
                    : style.image.replace('.png', '-desktop.png')
                }
                alt={style.alt}
